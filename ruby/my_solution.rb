...
# I worked on this challenge [by myself, with: Allyse Groover].
# We spent [#3] hours on this challenge.

# EXPLANATION OF require_relative
# Require is an absolute path, require relative is a relative path
# Require means that Ruby will look in a predetermined location for the file specified
# Require relative means you are starting from the current working directory, so that is where Ruby will look first. You dictate where the file is stored. 
...
# Require relative means you are starting from the current working directory, so that is where Ruby will look first. You dictate where the file is stored. 
# constants
# The difference between the two hash syntaxes is that the second one that ends with ":" are symbols
# Symbols are both functional and organizational. They are immutable values. If you have a lot of different pieces of information that have the same key value pairs symbols work as a label to define and unify that information. Also symbols are created once, so if many different pieces of information have only say a couple of sympols you only have two sympol objects, not many. This makes your files load faster. 